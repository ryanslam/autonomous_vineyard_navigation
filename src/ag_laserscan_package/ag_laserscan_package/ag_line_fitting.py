...
class AgVineyardRowLineFitting(Node):
    def __init__(self):
        ...
        self.cross_track_err_pub = self.create_publisher(
            Float32, "/ag/percept/crosstrack_err", 10
        )
    ...
    def get_polar_coordinates(self, scan, area_of_interest) -> dict:
        ...
        for degree, distance in enumerate(scan):
            if not (math.isinf(distance)) and self.is_in_area_of_interest(
                degree, area_of_interest
            ):
    ...
    def get_cartesian_coords(self, polar_coords) -> list:
        x = np.array(
            [
                self.get_x_from_polar_coords(degree, distance)
                for degree, distance in polar_coords.items()
            ]
        )
        y = np.array(
            [
                self.get_y_from_polar_coords(degree, distance)
                for degree, distance in polar_coords.items()
            ]
        )
        return [x, y]
    ...
    def determine_range(self, scan, area_of_interest) -> int:
        ...
        distance_to_point = math.sqrt(
            cartesian_coords[0][0] ** 2 + cartesian_coords[1][0] ** 2
        )
        ...
        print(
            "Slope: %f\nDistance: %f\nDegrees: %f\nDistance to point: %f"
            % (line_parameters[0], distance, heading, distance_to_point)
        )
        print("-------------------------------------------------------")
