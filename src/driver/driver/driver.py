...
class Driver(Node):
    ...
    def __init__(self):
        ...
        self.cross_track_sub = self.create_subscription(
            Float32, "/ag/percept/crosstrack_err", self.cross_track_callback, 10
        )

        self.cross_track_sub = self.create_subscription(
            Float32, "/ag/percept/heading_err", self.heading_callback, 10
        )

        self.angular_conf_sub = self.create_subscription(
            Float32, "/ag/percept/lidar_confidence", self.set_drive_flag, 10
        )
